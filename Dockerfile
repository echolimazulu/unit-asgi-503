...
RUN set -ex \
    && apt-get update \
    && apt-get install --no-install-recommends --no-install-suggests -y ca-certificates build-essential libssl-dev libpcre2-dev git \
    && mkdir -p /usr/lib/unit/modules /usr/lib/unit/debug-modules \
    && git clone https://github.com/alejandro-colomar/unit.git \
    && cd unit \
    && git checkout sock \
    && git fetch \
...
COPY things_asgi.py /opt/falcon/
